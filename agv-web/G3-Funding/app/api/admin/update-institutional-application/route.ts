...
export async function PATCH(request: NextRequest) {
  try {
    ...
    await adminDb.collection('institutional_applications').doc(appId).update(updateData);

    // If the application status is being updated to 'approved', also update any associated KOL profile status
    if (updates.status === 'approved') {
      try {
        // Get the application to find any associated KOL ID
        const appDoc = await adminDb.collection('institutional_applications').doc(appId).get();
