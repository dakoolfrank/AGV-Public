  ...
  async translate({ text, from, to }: { text: string; from: string; to: string }): Promise<string> {
    try {
      const response = await this.openai.responses.create({
        model: "gpt-5-nano",
        input: `You are a professional translator. Translate the following text from ${from} to ${to}. Maintain the original formatting, structure, and meaning. Return only the translated text without any explanations or additional content.\n\nText to translate: ${text}`,
        store: true,
      });
