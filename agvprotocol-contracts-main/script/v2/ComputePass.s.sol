...
contract DeployComputePass is Script {
    // Configuration constants
    string constant NAME = "ComputePass";
    string constant SYMBOL = "COMPUTE";
    ...
    function run() external {
        ...
        console.log("Deploying ComputePass implementation...");
        ComputePass implementation = new ComputePass();
    ...
    function testDeployedContract(address proxyAddress) internal view {
        ...
        ComputePass computepass = ComputePass(proxyAddress);

        string memory name = computepass.name();
        string memory symbol = computepass.symbol();
        uint256 totalSupply = computepass.totalSupply();
        string memory currentPhase = computepass.getCurrentPhase();
        uint256 remainingPublic = computepass.getRemainingPublicSupply();
        uint256 remainingReserved = computepass.getRemainingReservedSupply();
...
contract UpgradeComputePass is Script {
    function run() external {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        ...
        ComputePass newImplementation = new ComputePass();
...
contract ConfigureComputePass is Script {
    ...
    function run() external {
        ...
        console.log("Configuring ComputePass at:", proxyAddress);
        ...
        // 1. Set base URI
        string memory baseURI = "https://api.agvprotocol.com/metadata/computepass/"; // just for test
        ...
        if (!computepass.hasRole(AGENT_MINTER_ROLE, agent1)) {
        ...
        computepass.setSaleConfig(wlStart, wlEnd, true);
        console.log(" Updated sale config");
        console.log("  WL Start:", wlStart);
        console.log("  WL End:", wlEnd);

        // 4. withdraw funds to treasury'
        address treasury = computepass.treasuryReceiver();
        uint256 balance = address(computepass).balance;
        if (balance > 0) {
        ...
        console.log("Current Phase:", computepass.getCurrentPhase());
        console.log("Remaining Public Supply:", computepass.getRemainingPublicSupply());
        console.log("Treasury:", computepass.treasuryReceiver());
