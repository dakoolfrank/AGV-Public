...
contract DeployTreePass is Script {
    // Configuration constants
    string constant NAME = "TreePass";
    string constant SYMBOL = "TREE";
    ...
    function run() external {
        ...
        console.log("Deploying TreePass implementation...");
        TreePass implementation = new TreePass();
    ...
    function testDeployedContract(address proxyAddress) internal view {
        ...
        TreePass treepass = TreePass(proxyAddress);

        string memory name = treepass.name();
        string memory symbol = treepass.symbol();
        uint256 totalSupply = treepass.totalSupply();
        string memory currentPhase = treepass.getCurrentPhase();
        uint256 remainingPublic = treepass.getRemainingPublicSupply();
...
contract UpgradeTreePass is Script {
    function run() external {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        ...
        TreePass newImplementation = new TreePass();
...
contract ConfigureTreePass is Script {
    function run() external {
        uint256 privateKey = vm.envUint("PRIVATE_KEY");
        address proxyAddress = vm.envAddress("PROXY_ADDRESS");

        console.log("Configuring TreePass at:", proxyAddress);
        ...
        // 1. Set base URI
        string memory baseURI = "https://api.agvprotocol.com/metadata/treePass/"; // just for test
        ...
        treePass.setSaleConfig(wlStart, wlEnd, true);
        ...
        console.log("Current Phase:", treePass.getCurrentPhase());
        console.log("Remaining Public Supply:", treePass.getRemainingPublicSupply());
        console.log("Treasury:", treePass.treasuryReceiver());
